...
const IButton: React.FC<IButtonProps> = ({ content, link, className }) => {
  return (
    <Link
      href={link}
      className={`group relative inline-block w-52 overflow-hidden rounded-2xl border-[3px] border-iio bg-[#0000001d] px-6 py-2 text-iio transition-all duration-200 hover:bg-transparent ${className}`}
    >
      {/* Fluid fill effect */}
